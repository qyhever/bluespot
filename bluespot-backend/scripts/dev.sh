...
# 设置环境变量（默认为 dev）
export BLUESPOT_ENV="${2:-${BLUESPOT_ENV:-dev}}"

# 验证环境变量
if [[ ! "$BLUESPOT_ENV" =~ ^(dev|test|prod)$ ]]; then
    echo "❌ 错误: 无效的环境 '$BLUESPOT_ENV'，只允许: dev, test, prod"
    exit 1
fi

echo "🌍 当前环境: $BLUESPOT_ENV"
